...
export function AdminNav() {
  ...
  async function handleCreate() {
    ...
    try {
      await adminNavApi.create(newItem);
    ...
    <div className="p-6 lg:p-8 max-w-4xl mx-auto">
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}>
        ...
        <div className="space-y-3">
          {items.map((item) => (
            <div key={item.id}>
              <div className={`card p-4 flex items-center gap-4 ${!item.visible ? 'opacity-50' : ''}`}>
                <GripVertical className="w-4 h-4 text-slate-300 shrink-0 cursor-grab" />
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-medium text-slate-900 dark:text-white">{item.label_he} / {item.label_en}</p>
              ...
              {item.children.length > 0 && (
                <div className="ms-8 mt-2 space-y-2">
                  {item.children.map((child) => (
                    <div key={child.id} className={`card p-3 flex items-center gap-3 ${!child.visible ? 'opacity-50' : ''}`}>
                      <div className="w-1.5 h-1.5 rounded-full bg-slate-300" />
                      <div className="flex-1 min-w-0">
                        <p className="text-sm text-slate-700 dark:text-slate-300">{child.label_he} / {child.label_en}</p>
                        <span className="text-xs text-primary-500 font-mono">{child.href}</span>
                      </div>
                      <button onClick={() => toggleVisibility(child)} className="btn-ghost p-1">
                        {child.visible ? <Eye className="w-3.5 h-3.5 text-emerald-500" /> : <EyeOff className="w-3.5 h-3.5 text-slate-400" />}
                      </button>
                      <button onClick={() => handleDelete(child.id)} className="btn-ghost p-1 text-red-500">
